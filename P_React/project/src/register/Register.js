...
const Register = () => {
    ...
    const [message, setMessage] = useState('');  // 서버에서 받은 메시지를 저장하는 상태 변수
    const [username, setUsername] = useState('');  // 사용자가 입력한 아이디 값을 저장하는 상태 변수
    ...
    const checkId = async () => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:8080/underdog/Register?username=${username}`);
            
            // 서버로부터 받은 응답 메시지 설정
            setMessage(response.data.message);  // 응답에서 'message' 속성만 상태에 설정
        } catch (error) {
