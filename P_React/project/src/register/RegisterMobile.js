   ...
   const [employeeNumber, setEmployeeNumber] = useState(""); // 사원번호 입력값
    ...
    const checkId = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/register/id/check`, sendId,
    ...
    const sendRegisterData = async (e) => {
        ...
        if (idCheckResult) {
            if (pwCheckResult) {
                ...
                try {
                    const response = await axios.post(`${API_URL}/register/set`, RegisterDto, { headers: { 'Content-Type': 'application/json', 'Accept': 'text/plain'} });
    ...
    return (
            ...
            <form id={styles.registerForm} onSubmit={sendRegisterData}>
                ...
                사원번호
                <input className={styles.registerInput} type="text" placeholder="사원번호를 입력해 주세요" value={employeeNumber} onChange={(e)=>setEmployeeNumber(e.target.value)} required/>
                </div>
                {/* -----------------------준강 아이디, 아이디 중복 버튼 병합 -------------------*/}
