...
const handleLogin = async (event) => {
  ...
  try {
    ...
    if (response.data.pw_check) {
      ...
      // 로그인 정보 저장
      if (isSaveLogin) {
        localStorage.setItem("m_id", id); // 사용자 ID
        localStorage.setItem("e_name", userName); // 사용자 이름
        localStorage.setItem("authority", userAuthority); // 권한
      } else {
        sessionStorage.setItem("m_id", id); // 사용자 ID
        sessionStorage.setItem("e_name", userName); // 사용자 이름
        sessionStorage.setItem("authority", userAuthority); // 권한
      }
